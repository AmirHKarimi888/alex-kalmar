<template>
    <ul class="flex gap-1">
        <button @click="router.push(`/blog/${current - 1}`)"
        ...
        <button v-for="btn in btns" :key="btn" @click="router.push(`/blog/${btn}`)"
            class="border border-zinc-400 rounded-full w-10 h-10 flex justify-center items-center cursor-pointer disabled:bg-slate-300 disabled:opacity-50 disabled:cursor-auto"
            :class="current === btn ? 'bg-zinc-800 text-white' : ''" :disabled="btn > Math.ceil(allPosts.length / perPage)">
            {{ btn }}
        </button>
        <button @click="router.push(`/blog/${current + 1}`)"
            class="border border-zinc-400 rounded-full w-10 h-10 flex justify-center items-center cursor-pointer disabled:bg-slate-300 disabled:opacity-50 disabled:cursor-auto"
    ...
    <!-- <ul class="flex gap-1">
        <button @click="emit('changePage', current - 1, updateThreeMults)"
            class="border border-zinc-400 rounded-full w-10 h-10 flex justify-center items-center cursor-pointer disabled:bg-slate-300 disabled:opacity-50 disabled:cursor-auto"
            :disabled="current === 1">
            <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24">
                <path fill="currentColor" d="M11.67 3.87L9.9 2.1L0 12l9.9 9.9l1.77-1.77L3.54 12z" />
...
updateThreeMults(props.current);
</script>
